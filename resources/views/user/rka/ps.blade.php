...
<div class="col">
    <ul class="nav nav-pills justify-content-end">
        <li class="nav-item">
            <a href="{{route('user.rka.ps.add', Auth::user()->id)}}"
        ...
        <div class="card">
            <div class="card-body bg-transparent">
                <div class="row align-items-center">
                    <div class="col">
                        <h6 class="text-uppercase ls-1 mb-1">{{ $ps->jenis }}</h6>
                        <h3 class="h3 mb-0">{{ $ps->jenis_barang }}</h3>
                        <h6 class="mt-1">{{ $ps->created_at }}</h6>
                    </div>
                    <div class="col">
                        <ul class="nav nav-pills justify-content-end">

                            <li class="nav-item mr-2 mr-md-0">
                                <a href="{{route('user.rka.ps', Auth::user()->id)}}"
                            ...
                            @if($ps->is_approve_admin_zona == 0)
                            <li class="nav-item mr-2 mr-md-0">
                                <button onclick='proccessAdminZona({{$ps->id}})'
                                    class="btn nav-link py-2 px-3 bg-success active">
                                    <span class="d-md-block"> Approve Admin Zona</span>
                                </button>
                            </li>
                            @endif
                            @if($ps->is_approve_admin_zona == 1)
                            <li class="nav-item mr-2 mr-md-0">
                                <a href="" class="nav-link py-2 px-3 bg-success active">
                            ...
                            @if($ps->is_approve_super_admin == 0)
                            <li class="nav-item mr-2 mr-md-0">
                                <button onclick='proccessSuperAdmin({{$ps->id}})'
                                    class="btn nav-link py-2 px-3 bg-success active">
                                    <span class="d-md-block"> Approve Super Admin</span>
                                </button>
                            </li>
                            @endif
                            @if($ps->is_approve_super_admin == 1)
                            <li class="nav-item mr-2 mr-md-0">
                                <a href="" class="nav-link py-2 px-3 bg-success active">
                            ...
                            @endif
                            @endif
                            @if ($ps->is_upload_nota==0)
                            <li class="nav-item mr-2 mr-md-0">
                                <a href="{{route('user.rka.nota.add', $ps->id)}}"
                                    class="nav-link py-2 px-3 bg-success active">
                                    <span class="d-md-block">Upload Nota</span>
                                </a>
                            </li>
                            @elseif($ps->is_upload_nota==1 && $ps->file_nota!=null)
                            <li class="nav-item mr-2 mr-md-0">
                                <a href="{{route('user.rka.nota.download', $ps->file_nota)}}"
                                    class="nav-link py-2 px-3 bg-success active">
                                    <span class="d-md-block">Download Nota</span>
                                </a>
                            </li>
                            <li class="nav-item mr-2 mr-md-0">
                                <a <a href="{{route('user.rka.ps.kuitansi', Auth::user()->id)}}"
                                    class="nav-link py-2 px-3 bg-success active">
